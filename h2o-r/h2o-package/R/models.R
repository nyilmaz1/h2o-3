...
#' @param add_missing_NA A logical specifying whether missing value should be included in the Features.  This is only enabled if there are missing values in the features.
...
h2o.partialPlot <- function(object, data, cols, destination_key, nbins=20, plot = TRUE, plot_stddev = TRUE, weight_column_index=-1, add_missing_NA=FALSE) {
  ...
  if(!is.logical(add_missing_NA)) stop("add_missing_NA must be a logical value")
  ...
  if (is.numeric(weight_column_index)) {
    if (weight_column_index >0 && weight_column_index>h2o.ncol(data))
      stop("weight_column_index exceeds number of columns in your data frame.")
  } else { # weight_column_index is column name
    if (!weight_column_index %in% h2o.names(data))
      stop("weight_column_index should be one of your columns in your data frame.")
    else
      weight_column_index = match(weight_column_index, h2o.names(data))-1
  }
  ...
  parms$nbins <- nbins
  parms$weightColumnIndex <- weight_column_index
  parms$addMissingNA <- add_missing_NA
